    ...
    var ROUNDING_FNS = {
        ...
        function AutoSizer() {
            ...
            _defineProperty(_this, "_onResize", function() {
                var height, zoom, paddingRight, paddingTop, boundingRect, _this$props = _this.props, disableHeight = _this$props.disableHeight, disableWidth = _this$props.disableWidth, onResize = _this$props.onResize;
                _this._parentNode && (_this$props = ROUNDING_FNS[_this$props.roundingMode] || Math.ceil, 
                boundingRect = _this._parentNode.getBoundingClientRect(), zoom = (node => {
                    if ("number" == typeof node.currentCSSZoom && 0 < node.currentCSSZoom) return node.currentCSSZoom;
                    for (var zoom = 1, el = node, win = node.ownerDocument && node.ownerDocument.defaultView || window; el && 1 === el.nodeType; ) {
                        var own = parseFloat(win.getComputedStyle(el).zoom);
                        own && !isNaN(own) && (zoom *= own), el = el.parentElement;
                    }
                    return zoom;
                })(_this._parentNode) || 1, height = _this$props(boundingRect.height / zoom) || 0, 
                _this$props = _this$props(boundingRect.width / zoom) || 0, boundingRect = (_this._window || window).getComputedStyle(_this._parentNode) || {}, 
                zoom = parseInt(boundingRect.paddingLeft, 10) || 0, paddingRight = parseInt(boundingRect.paddingRight, 10) || 0, 
                paddingTop = parseInt(boundingRect.paddingTop, 10) || 0, boundingRect = parseInt(boundingRect.paddingBottom, 10) || 0, 
                !disableHeight && _this.state.height !== height - paddingTop - boundingRect || !disableWidth && _this.state.width !== _this$props - zoom - paddingRight) && (_this.setState({
                    height: height - paddingTop - boundingRect,
                    width: _this$props - zoom - paddingRight
                }), onResize({
                    height: height,
                    width: _this$props
                }));
